...
def cumulative(D, E, S, kind="all"):
    """-> {animal: [cumulative seizure MINUTES after each baseline day 1..21]}

    kind: 'all' | 'convulsive' | 'non_convulsive'
    """
    import collections
    sec = collections.defaultdict(float)
    for ci, a, t, d in D.ev:
        if D.wk.get(ci) in BASE and (kind == "all" or t == kind):
            sec[(a, D.day[ci])] += d
    out = {}
    for a in E + S:
        run, series = 0.0, []
...
def main() -> int:
    ...
    ws3["A1"].font = BOLD
    ws3["A3"] = ("PRISM TABLE TYPE:  XY, X = day, one Y column per animal "
                 "(Y = single values, no replicates)")
    ws3["A3"].font = BOLD
    ws3["A4"] = ("Two blocks below: CONVULSIVE ONLY and ALL SEIZURES. Use one. "
                 "Plot each animal as a thin grey line, group medians bold.")
    r = 6
    endpoints = {}
    for kind, label in (("convulsive", "BLOCK 1 — CONVULSIVE seizures only "
                         "(recommended: matches the survival panel)"),
                        ("all", "BLOCK 2 — ALL seizures "
                         "(convulsive + non-convulsive)")):
        cum = cumulative(D, E, S, kind)
        ...
        endpoints[kind] = (mw(fa, fb), sum(1 for a in S if cum[a][20] == 0))
        r += 1

    (pc_all, zc), (pa_all, za) = endpoints["convulsive"], endpoints["all"]
    put_notes(ws3, r, [
        "STATISTICS TO RUN IN PRISM",
        "  The curves are descriptive. Test the ENDPOINT (day 21 total) only:",
        "  Analyze → Column analyses → t test (and nonparametric tests)",
        "         → 'Mann-Whitney test' (unpaired, two-tailed), EGFP vs SV2A.",
        f"  CONVULSIVE ONLY: median EGFP {pc_all[0]:.1f} min, SV2A "
        f"{pc_all[1]:.1f} min, p = {pc_all[2]:.4f}",
        f"  ALL SEIZURES:    median EGFP {pa_all[0]:.1f} min, SV2A "
        f"{pa_all[1]:.1f} min, p = {pa_all[2]:.4f}",
        ...
        "",
        "WHICH BLOCK: convulsive-only is recommended. Its "
        f"{zc}/{len(S)} SV2A animals that",
        "never seize are the SAME animals censored in the survival panel, so the",
        f"two panels cross-reference. All-seizures gives {za}/{len(S)} at zero,",
        "which a reader then has to reconcile against the survival panel.",
        "",
        "PLOT MEDIAN, NOT MEAN +/- SEM. The SV2A group is bimodal — several",
        "animals at exactly 0 for all 21 days and 2 non-responders as high as",
        "EGFP — so the mean is dragged up and the error bars overlap, which",
        "understates a difference the medians show clearly.",
        "",
        "ZEROS ARE REAL. A flat line along the axis means that animal never had",
        "a seizure; it is not a plotting floor. Use a LINEAR y-axis — do not",
        "log-transform, or the flat-at-zero animals cannot be drawn.",
        "",
        "Y-AXIS LABEL: 'Cumulative convulsive seizure time (min)' for block 1,",
        "or 'Cumulative seizure time (min)' for block 2. State which in the legend.",
    ])
